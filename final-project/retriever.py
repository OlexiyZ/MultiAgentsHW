...
def reciprocal_rank_fusion(
    *ranked_lists: list[Document], k: float = 60.0, limit: int = 20
    ...
    """Merges ranked document lists with reciprocal rank fusion and returns the top `limit` docs.
    Об'єднує ранжовані списки документів методом reciprocal rank fusion і повертає топ `limit` записів."""
...
def _format_hits(documents: list[Document]) -> str:
    """Formats retrieved documents as numbered Markdown snippets with source labels.
    Форматує знайдені документи як нумеровані Markdown-фрагменти з позначкою джерела."""
