...
class Settings(BaseSettings):
    ...
    supervisor_model_name: str = Field(
        default="gpt-4o-mini",
        validation_alias=AliasChoices("SUPERVISOR_MODEL_NAME", "MODEL_NAME", "supervisor_model_name"),
    )
    planner_model_name: str = Field(
        default="gpt-4o-mini",
        validation_alias=AliasChoices("PLANNER_MODEL_NAME", "MODEL_NAME", "planner_model_name"),
    )
    research_model_name: str = Field(
        default="gpt-4o-mini",
        validation_alias=AliasChoices("RESEARCH_MODEL_NAME", "MODEL_NAME", "research_model_name"),
    )
    critic_model_name: str = Field(
        default="gpt-4o-mini",
        validation_alias=AliasChoices("CRITIC_MODEL_NAME", "MODEL_NAME", "critic_model_name"),
    )
    ...
    acp_http_timeout: float = Field(
        default=600.0,
        validation_alias=AliasChoices("ACP_HTTP_TIMEOUT", "acp_http_timeout"),
        description="Seconds for Supervisor HTTP calls to the ACP server (planner/researcher/critic runs).",
    )

    search_mcp_url: str = Field(
        default="http://127.0.0.1:8901/mcp",
        validation_alias=AliasChoices("SEARCH_MCP_URL", "search_mcp_url"),
    )
    report_mcp_url: str = Field(
        default="http://127.0.0.1:8902/mcp",
        validation_alias=AliasChoices("REPORT_MCP_URL", "report_mcp_url"),
    )
    acp_base_url: str = Field(
        default="http://127.0.0.1:8903",
        validation_alias=AliasChoices("ACP_BASE_URL", "acp_base_url"),
    )
    search_mcp_port: int = Field(default=8901, validation_alias=AliasChoices("SEARCH_MCP_PORT"))
    report_mcp_port: int = Field(default=8902, validation_alias=AliasChoices("REPORT_MCP_PORT"))
    acp_port: int = Field(default=8903, validation_alias=AliasChoices("ACP_PORT"))
    ...
    log_destination: str = Field(
        default="stderr",
        validation_alias=AliasChoices("LOG_DESTINATION", "LOG_TO", "log_destination"),
    )
    log_file: str = Field(
        default="logs/lesson9.log",
...
def configure_logging() -> None:
    ...
    if dest in {"file", "both"}:
        raw = Path(settings.log_file.strip() or "logs/lesson9.log")
